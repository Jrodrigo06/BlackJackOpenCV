...
While this function looks complicated, it is quite simple. The left side is the value function and it's a recursive function where it finds the maximum future value over all possible actions. The summation of probablities is there due to stochasticity of the environment. This is due to the fact that the next state is not guarnteed with the action taken, for example in blackjack the next state isn't guranteed with a hit, as the next card pulled is random. The summation accounts for all possible future states, weighted by their probabilities, in order to evaluate the expected value of taking each action.